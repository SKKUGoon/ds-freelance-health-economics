"""Stage 2 training — ts_supply_ts_latent mode (supply-history + latent rollout)."""
...
def train_supply_head_indexed(
    ...
    """
    Train one supply head on a subset of targets (ts_supply_ts_latent mode).
    ...
    if not cfg.use_supply_history:
        raise ValueError(
            "ts_supply_ts_latent mode requires use_supply_history=True in config."
        )
    ...
    for epoch in range(1, cfg.epochs_supply + 1):
        ...
        for batch in train_loader:
            if len(batch) == 3:
                ...
                raise ValueError(
                    "ts_supply_ts_latent training expects batches of 3 or 4 elements. "
                    f"Got {len(batch)}."
                )
        ...
        if patience is not None and stale_epochs >= int(patience):
            print(
                f"[stage2:ts_supply_ts_latent:{head_type}] Early stopping at epoch={epoch}; "
    ...
    if best_state is not None:
        ...
        print(
            f"[stage2:ts_supply_ts_latent:{head_type}] Loaded best checkpoint from "
