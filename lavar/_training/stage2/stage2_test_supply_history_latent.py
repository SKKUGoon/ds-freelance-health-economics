"""Stage 2 training — supply_history_latent mode (supply-history + latent rollout)."""
...
def _resolve_softplus_beta(cfg: LAVARConfig, epoch: int) -> float:
    mode = getattr(cfg, "stage2_delta_nonneg_mode", "clamp")
    beta_start = float(getattr(cfg, "stage2_softplus_beta_start", 1.0))
    beta_end = float(getattr(cfg, "stage2_softplus_beta_end", 8.0))
    if mode != "softplus_annealed":
        return beta_end

    if cfg.epochs_supply <= 1:
        return beta_end

    progress = float(epoch - 1) / float(cfg.epochs_supply - 1)
...
def train_supply_head_indexed(
    ...
    """
    Train one supply head on a subset of targets (supply_history_latent mode).

    Requires use_supply_history=True so that x_past is augmented with y columns
    (encoder input dim = Dx + Dy).
    """
    y_indices = list(y_indices)
    if len(y_indices) == 0:
        return None

    if not cfg.use_supply_history:
        raise ValueError(
            "supply_history_latent mode requires use_supply_history=True in config."
        )
    ...
    weight_decay = getattr(cfg, "weight_decay_supply", 0.0)
    opt = torch.optim.AdamW(model.supply_head.parameters(), lr=cfg.lr_supply,
    ...
    for epoch in range(1, cfg.epochs_supply + 1):
        if head_type == "delta_mse":
            ...
            model.set_softplus_beta(_resolve_softplus_beta(cfg, epoch))

        model.train()
        for batch in train_loader:
            if len(batch) == 3:
                x_past, _x_future, y_future = batch  # x_past: (B, p+1, Dx+Dy)
                y0 = None
            elif len(batch) == 4:
                x_past, _x_future, y0, y_future = batch  # y0: (B, Dy)
            else:
                raise ValueError(
                    "supply_history_latent training expects batches of 3 or 4 elements. "
                    f"Got {len(batch)}."
                )
            x_past = x_past.to(device)  # (B, p+1, Dx+Dy)
        ...
        if patience is not None and stale_epochs >= int(patience):
            print(
                f"[stage2:supply_history_latent:{head_type}] Early stopping at epoch={epoch}; "
    ...
    if best_state is not None:
        ...
        print(
            f"[stage2:supply_history_latent:{head_type}] Loaded best checkpoint from "
